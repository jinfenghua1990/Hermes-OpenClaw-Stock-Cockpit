        ...
        try:
            with open(decision_log, 'r', encoding='utf-8') as f:
                ...
                for decision in data.get("decisions", []):
                    ...
                    if today in decision_time:
                        if decision.get("ai_autonomous", False):
...
def main():
    ...
    if not is_safe:
        print("\n⚠️ 警告: Phase-2.4B-Stable稳定运行期已违反禁止规则")
        print("请立即停止违规操作，保持系统只读观察模式")
    
    print("\nPhase-2.4B-Stable 稳定运行规则:")
    print("  1. 禁止自动学习")
